...
namespace BKISystemAdmin.Manager
{
    public class CRoleManager
    {
        ...
        public IEnumerable<CChucNangModel> GetAllChucNangByRoleForMenu(Guid ip_guid_role)
        {
            var uow = new UnitOfWork();
            var v_lst_quyen_chuc_nang = uow.Repository<HT_PHAN_QUYEN_CHUC_NANG>()
                .Query().Include(x => x.HT_PHAN_QUYEN_CHUC_NANG_CHILDREN)
                .Filter(x => x.ID_HT_USER_GROUP == ip_guid_role && !x.ID_CHUC_NANG_CHA.HasValue).Get();

            var op_results = v_lst_quyen_chuc_nang.Select(x =>
            {
